...
const SandboxIDE = ({ activeTask, courseType, onBack }) => {
  const currentTask = useMemo(() => {
    ...
    const courseId = courseType === 'python' ? 1 : 3;
    const course = coursesData[courseId] || coursesData[1];
    const firstModule = course?.modules?.[0];
    const assign = firstModule?.assignment;

    if (assign) {
      const isStringAssign = typeof assign === 'string';
      return {
        title: isStringAssign ? `${firstModule.title} Assignment` : (assign.title || `${firstModule.title} Assignment`),
        module: `${firstModule.title}: ${firstModule.subtitle || ""}`,
        description: isStringAssign ? assign : (assign.objective || firstModule.description || ""),
        objectives: isStringAssign
          ? [assign]
          : (assign.steps || [assign.objective || "Complete the assignment."]),
        hint: isStringAssign
          ? "Review the lesson materials and apply the concepts covered in this module."
          : (Array.isArray(assign.hints) ? assign.hints.join('\n\n') : (assign.hints || ""))
      };
    }
    ...
    if (!window.Sk) {
      setOutput((prev) => [
        ...prev,
        {
          type: "error",
          message:
            "Python interpreter loading... please try again in a moment.",
        },
      ]);
      return;
    }
